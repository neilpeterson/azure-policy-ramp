...
resource singingCertificate 'Microsoft.Compute/virtualMachines/extensions@2020-06-01' = if (!empty(signCert)) {
  ...
  properties: {
    ...
    autoUpgradeMinorVersion: true
    protectedSettings: {
      commandToExecute: 'powershell.exe Invoke-WebRequest "${signCert}" -OutFile c:\\cert.cer && powershell.exe Import-Certificate -FilePath c:\\cert.cer -CertStoreLocation Cert:\\LocalMachine\\TrustedPublisher\\ && powershell.exe Remove-Item c:\\cert.cer -Force'
    }
  }
}
